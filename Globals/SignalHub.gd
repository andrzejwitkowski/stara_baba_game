...
func emit_on_add_new_explosion(new_position: Vector3) -> void:
	on_add_new_explosion.emit(new_position)

func emit_on_add_new_scene(ob: Node3D, new_position: Vector3) -> void:
	on_add_new_scene.emit(ob, new_position)
